    ...
    char **phr)
{
  ...
  while (n < TH_DECLARED_MAX) {
    char *k, *b, *t, *f;
    k = thJstrAt(&p); if (!k) break; free(k);
    b = thJstrAt(&p);
    k = thJstrAt(&p); free(k);
    t = thJstrAt(&p);
    k = thJstrAt(&p); free(k);
    f = thJstrAt(&p);
    if (!b || !t || !f) { free(b); free(t); free(f); break; }
...
thModel *thLoad(const char *dir, char *err, size_t errLen)
{
  ...
  m->thVersionMargin = (float) thJnum(j, "version_margin", 2.0);
...
void thFree(thModel *m)
{
  ...
  { int q;
    ...
    for (q = 0; q < m->nDecl; q++) {
      free(m->declBase[q]); free(m->declTo[q]); free(m->declPhr[q]);
    } }
  free(m->variantBase);
  ...
  free(m->reqPhr); free(m->reqSib); free(m->conIn); free(m->conNear); free(m->conHolder); free(m->unver);
  { int q;
    for (q = 0; q < m->nClause; q++) { free(m->clauseKey[q]); free(m->clausePhr[q]); }
    for (q = 0; q < m->nFamily; q++) { free(m->famName[q]); free(m->famKeys[q]); }
...
static int thCtxOk(const char *t, size_t at)
{
  ...
  if (pre[n - 1] == 'v') { /* a bare "v" prefix */
    /* A word character before the v means no version is stated: the V1 in
       "License_CeCILL-B_V1-fr.html" is part of a filename. */
    if (n == 1 || (!thWordBefore(pre, n - 1) && pre[n - 2] != '_'))
      return 1;
  }
  for (i = 0; TH_LICWORD[i]; i++) {
    size_t L = strlen(TH_LICWORD[i]);
    if (n >= L && !strcmp(pre + n - L, TH_LICWORD[i])) return 1;
  }
  return 0;
}
...
/**
 * \brief A technology-named license claimed on the bare word alone.
 *
 * "import python" and "<?php" are code. The word must sit next to "license",
 * since a window straddling a license block puts them a few words apart.
 */
static int thTechWordOnly(const char *name, const char *t)
{
  static const char *tech[] = {"python","php","ruby","perl","zlib","curl","json",
      "unicode","tcl","vim","icu","x11","sqlite","bzip2","openssl","ncurses",
      "freetype","fair","loop","doc", NULL};
  static const char *generic[] = {"license","licence","public","general","version",
      "software","free","open","source","only","later","plus","the","of","and","or",
      "v", NULL};
  char word[64];
  int nw = 0, g;
  size_t j = 0, n = strlen(name);
  char cur[64];
  size_t c = 0;
  word[0] = '\0';
  for (j = 0; j <= n; j++) {
    unsigned char ch = (unsigned char) name[j];
    if (isalnum(ch)) { if (c < sizeof(cur) - 1) cur[c++] = (char) tolower(ch); continue; }
    cur[c] = '\0';
    if (c > 2) {
      int skip = 0;
      for (g = 0; generic[g]; g++) if (!strcmp(cur, generic[g])) { skip = 1; break; }
      if (!skip) { nw++; if (nw == 1) snprintf(word, sizeof(word), "%s", cur); }
    }
    c = 0;
  }
  if (nw != 1) return 0;
  for (g = 0; tech[g]; g++) if (!strcmp(word, tech[g])) break;
  if (!tech[g]) return 0;
  /* Only "<word> license" or "license <word>", whole words: "licensor" after
     the tech word let OpenSSL through as a license. */
  { const char *p;
    size_t wl = strlen(word);
    for (p = t; (p = strstr(p, word)) != NULL; p += wl) {
      const char *after = p + wl;
      if ((p == t || !isalnum((unsigned char) p[-1])) &&
          !isalnum((unsigned char) *after)) {
        while (*after == ' ' || *after == ':') after++;
        if (!strncmp(after, "license", 7) || !strncmp(after, "licence", 7))
          return 0;
        if (p - t >= 8) {
          const char *before = p - 1;
          while (before > t && (*before == ' ' || *before == ':')) before--;
          if (before - t >= 6 && (!strncmp(before - 6, "licence", 7) ||
                                  !strncmp(before - 6, "license", 7))) return 0;
        }
      }
    }
  }
...
{
  if (!thTechWordOnly(m->classes[cls], t)) return 0;
...
static int thClassifyN(thModel *m, const char *text, thResult *out, char **norm)
{
  ...
  { int rank[TH_CAND_MAX + 3], nr = m->thCandidates + 3, q, r2, w;
    ...
    for (q = 0, w = 0; q < nr && rank[q] >= 0; q++)
      if (!thTechWordMirage(m, rank[q], t)) rank[w++] = rank[q];
  ...
  /* version constraint: only when the classifier was not already decisive */
  if (out->margin <= m->thVersionMargin) {
    char base[192], ver[64], suf[16];
    ...
    for (i = 0; i < m->nClasses; i++) {
      ...
      if (strcmp(b2, base) || !v2[0]) continue;
    ...
    if (nHit == 1) {
      if (strcmp(m->classes[hit], pick) != 0) {
        ...
        method = strcmp(method, "ml+suffix") == 0 ? "ml+suffix+version" : "ml+version";
      }
      ...
      { int bestOther = -1;
        ...
        if (bestOther >= 0 && ls[top] - ls[bestOther] > out->margin)
    ...
    char *buf, size_t bufLen)
{
  ...
  line = malloc(cap * sizeof(*line));
  if (!line) return -1;
  line[n++] = p;
  while (*p) {
    /* Universal newlines, as the reference reads a file: a lone carriage
       return ends a line too. Splitting on \n alone leaves a classic-Mac
       source file as one line, so it is never windowed and the two
       implementations answer differently on it. */
    if (*p == '\n' || (*p == '\r' && p[1] != '\n')) {
      if (n == cap) {
    ...
    char *buf, size_t bufLen, size_t *used)
{
  ...
  for (i = 0; i < m->nRef; i++) {
    ...
    /* A row that names a version yields to a version the file states:
       "distributed under the GNU General Public License, version 2, or any
       higher version" is not GPL-1.0-or-later, whatever the head made of
       its wording. */
    if (strpbrk(name, "0123456789") && thVersionStated(t + hit.rm_eo)) continue;
matched:
    for (k = 0; see[k]; k++)
...
static int thConditionGone(const char *item)
{
  ...
  for (k = 0; gone[k]; k++) {
    ...
    if (strncmp(item, gone[k], len) == 0 && (item[len] == ' ' || item[len] == '\0')) return 1;
  }
  return 0;
}
 ...
 */
static int thFewestConditions(const char *nt, long *longest, int *any)
{
    ...
    thResult *out, int found)
{
  ...
  fewest = thFewestConditions(nt, &longest, &any);
  for (a = 0; a < found; a++) {
    ...
    /* more conditions than the licence numbers, or -- the mirror -- a list
       ...
       all: the licence's grant sentence without its conditions, which
       zlib's "Permission is granted to anyone to use this software for any
       purpose ... and redistribute it freely" is without its three
       restrictions */
    int beyond = own > 0 && fewest > own;
    int shortOf = span > 0 && fewest == 0
        && (!any || (longest > 0 && (double) longest < TH_CONDITIONS_SHARE * (double) span));
    if ((beyond || shortOf)
    ...
    const char *text, const char *nt)
{
  ...
  for (h = 0; m->conNear[ci][h] && nFit < TH_HOLDERS_MAX; h++) {
    ...
    for (b = 0; b < found; b++)
      if (out[b].license && strcmp(out[b].license, outer) == 0) { claimed = 1; break; }
    if (claimed) continue;
    ...
    char *buf, size_t bufLen)
{
  ...
  int nNotice = 0;
  /* The file normalised once, for every rule that reads it whole. */
  char *fileNorm = NULL;
#define TH_FILE_NORM() (fileNorm ? fileNorm : (fileNorm = thNormalise(text)))
  ...
  while (*p) {
    /* Universal newlines, as the reference reads a file: a lone carriage
       return ends a line too. Splitting on \n alone leaves a classic-Mac
       source file as a single line, so it is never windowed and the two
       implementations answer differently on it. */
    if (*p == '\n' || (*p == '\r' && p[1] != '\n')) {
      if (n == cap) {
        const char **t = thGrow(line, (cap *= 2) * sizeof(*line));
    ...
    if (nt) {
      ...
      if (covered && mine) {
        ...
        for (a = 0; a < found; a++) {
          ...
          int drop = 0;
          if (ci >= 0 && m->bodyPhr[ci] && m->nameVar
              && thNameEvidence(m, ci, text, nt) < m->thNameMin) {
            size_t total = 0, shared = 0, i2;
            int k;
            memset(mine, 0, len + 1);
            for (k = 0; m->bodyPhr[ci][k]; k++) {
              const char *ph = m->bodyPhr[ci][k];
            ...
            for (i2 = 0; i2 < len; i2++)
              if (mine[i2]) { total++; if (covered[i2]) shared++; }
  ...
  if (m->reqPhr) {
    ...
    if (nt) {
      int a, b, r;
      for (a = 0; a < found; a++) {
        int ia = -1, beaten = 0, mine = 0;
        for (b = 0; b < m->nClasses; b++)
          if (strcmp(m->classes[b], out[a].license) == 0) { ia = b; break; }
        if (ia < 0 || !m->reqPhr[ia]) continue;
        for (r = 0; m->reqPhr[ia][r]; r++)
        ...
        for (r = 0; m->reqSib[ia] && m->reqSib[ia][r] && !beaten; r++) {
          int c, is = -1, sr;
          ...
          if (is < 0) continue;
          /* A license with no distinctive phrases has none because a sibling
             contains all of it, so being reported is itself the evidence. */
          if (!m->reqPhr[is]) { beaten = 1; break; }
          for (sr = 0; m->reqPhr[is][sr]; sr++)
            if (strstr(nt, m->reqPhr[is][sr])) { beaten = 1; break; }
        } }
        if (beaten) {
          int k;
  ...
  /* One claim per family: the members contradict each other, and independently
     scored windows produce both. Ties survive. */
  { int a, b;
    for (a = 0; a < found; a++) {
      char ba[192], va[64], sa[16];
      int top = out[a].score;
  ...
  /* A licence named but not quoted cannot settle which version it is: the
     version-less name belongs to the earliest release, and where a later one
     exists that is a guess. The licence's own text decides instead. */
  if (m->unver && found > 0) {
    const char *nt = TH_FILE_NORM();
    if (nt) {
      int a, w = 0;
      for (a = 0; a < found; a++) {
        ...
        for (ci = 0; ci < m->nClasses; ci++)
          if (strcmp(m->classes[ci], out[a].license) == 0) {
            if (m->unver[ci]
                && thBodyEvidence(m, ci, nt, m->thBodyMin, 0) < m->thBodyMin)
              keep = 0;
            break;
          }
        if (keep) out[w++] = out[a];
...
static char *thOwnWords(const char *text)
{
  ...
  const char *p;
  if (!at || (at > text && at[-1] != '\n')) return NULL;
  if (!strstr(text, "\"Project-Id-Version:") && !strstr(text, "\"Content-Type:"))
    return NULL;
