...
static char *thSlurp(const char *p, size_t cap)
{
  ...
  /* The file is text: nothing readable carries a NUL in its first block. A
     NUL further in is an index a generator appended -- texinfo writes one
     after the manual -- and a C string would end the file there, silently
     dropping everything past it. Blanked, not cut, so every offset still
     points where it did. */
  for (i = 0; i < n; i++)
    if (b[i] == '\0') b[i] = ' ';
  return b;
